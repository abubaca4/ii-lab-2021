...
def find_path(start_point: int, matrix: np.array):
    ...
    person_priority = sorted(range(0, person_count), key= lambda people: np.sum(matrix[people]))
    return find_path_r(start_point, start_point, np.zeros(person_count), matrix, [], person_priority)
...
def find_path_r(current: int, start_point: int, is_visited: np.array, matrix: np.array, way: list, priority :list):
    ...
    for i in priority:
        if not is_visited[i] and matrix[current][i]:
            result = find_path_r(i, start_point, is_visited.copy(), matrix, way.copy(), priority)
            if result != False:
                return result
    return False